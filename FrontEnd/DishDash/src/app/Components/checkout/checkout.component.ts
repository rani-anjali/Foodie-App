...
//Now sending it to backend
  ...
  //checking for address selected or not
  checkAddressSelected() {
    console.log("Check method called");
   
    if(!this.primaryAddress || this.activeCustomer.customerAddress.length ==0) {
this.showAddressError = true;
    }

    if(this.cartItems.length == 0 ) {
      this.showCartErrorMessage = true;
    }
  }
  ...
  canDeactivate():boolean {
      ...
      if(this.cartItems.length > 0 && !this.orderPlacing ) {

        // const dialogRef = this.dialog.open(ConfirmDialogComponent, {
        //   data : {
        //   title:'Wait',
        //   message:'Your order is not placed yet, do you want to visit another page?'
        //   }
        // })
        ...
        //   }
        // )
       const reponse = confirm("You have not placed order yet, do you want to navigate?")
       return reponse;
      }
      return true;
  }
